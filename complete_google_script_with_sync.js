// Google Apps Script Code (Script.gs) - WITH SYNC FUNCTION
const CONFIG = {
...
function doPost(e) {
  try {
    ...
    } else if (action === 'sync_to_crm') {
      return syncExistingDataToCRM();
    }
...
// NEW FUNCTION: Sync existing Google Sheets data to CRM
function syncExistingDataToCRM() {
  try {
    const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const leadData = {
        ...
        email: row[3] || '',
        unit_size: row[4] || '',
        project_name: row[5] || CONFIG.PROJECT_SUBJECT,
        ip: row[6] || 'N/A',
        timestamp: row[0] || ''
      };
      
      try {
        const success = sendToCRM(leadData);
        if (success) {
          syncedCount++;
        } else {
          errorCount++;
    ...
    const result = `Sync complete: ${syncedCount} synced, ${errorCount} errors`;
    console.log(result);
...
// MANUAL TRIGGER: Run this function manually to sync all data
function manualSyncToCRM() {
  return syncExistingDataToCRM();
}