...
def import_all_members():
    # Clear existing data
    print("Clearing existing team members and users...")
    TeamMember.objects.all().delete()
    User.objects.exclude(is_superuser=True).delete()
    
    # All 321 team members data
    all_team_data = [
    ...
    print("Pass 1: Creating top-level members...")
    for member_data in all_team_data:
        if not member_data["parent"]:
            user, member, created = create_member_with_user(member_data)
            if created:
                imported_count += 1
                print(f"Created: {member.name} (user: {user.username})")
    
    print("Pass 2-5: Creating members with parents...")
        ...
        for member_data in all_team_data:
            if member_data["parent"] and not TeamMember.objects.filter(email=member_data["email"]).exists():
                try:
                    ...
                    user, member, created = create_member_with_user(member_data, parent)
                    if created:
                        imported_count += 1
                        print(f"Pass {pass_num} - Created: {member.name} (user: {user.username})")
                except TeamMember.DoesNotExist:
                    continue
    
    print(f"\nSuccessfully imported {imported_count} team members with user accounts")
